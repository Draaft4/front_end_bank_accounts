...
class CAJAAccountPage {
  ...
  void fetchData() async {
    ...
    List<MovimientoContable> allMovimientos =
        (await apiService.fetchData("C092")).cast<MovimientoContable>();
  ...
  Widget dasboard(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cuenta Cooperativa CAJA C092'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () {
              Get.to(() => AccountMoveForm(),
                  arguments: {'account_origin': 'C092'});
            },
          ),
          IconButton(
