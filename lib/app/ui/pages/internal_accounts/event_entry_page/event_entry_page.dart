...
class EventEntryPage {
  ...
  void fetchData() async {
    ...
    List<MovimientoContable> allMovimientos =
        (await apiService.fetchDataInternal("INGRESO DE EVENTOS"))
    ...
    movimientos.value = allMovimientos.where((movimiento) {
      ...
      if (cuentaFilterController.text.isNotEmpty) {
        matches &= movimiento.cuenta
                ?.contains(cuentaFilterController.text.toUpperCase()) ??
            false;
      }
  ...
  Widget dasboard(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cuenta Ingreso de Eventos'),
        actions: [
          CupertinoButton(
              child: const Row(
                children: [
